...
import { DragNDrop, Simple } from '../Lists'
import { Container, Header, HeaderName, HeaderButtons, Button} from './styles'
...
function Atendimento(props) {
    ...
    return (
        <Container>
            <Header>
                <HeaderName>{props.tecnico}</HeaderName>
                <HeaderButtons>
                    {ordenado && <Button onClick={rollbackOrders} hover={colors.azul} title={'Desfazer'}> <MenuIcon name='desfazer' margin='0.5' /> </Button>}
                    {ordenado && <Button hover={colors.azul} title={'Salvar'}> <MenuIcon name='salvar' margin='0.5' /> </Button>}
                    <Button onClick={() => setExpandido(!expandido)} hover={colors.azul} title={expandido? 'Recolher' : 'Expandir'}> 
                        <MenuIcon name={expandido? 'arrow_up' : 'arrow_dwn'} margin='0' />
                    </Button>
                </HeaderButtons>
            </Header>
            {props.draggable? <DragNDrop {...props} {...{expandido, handleOnDragEnd, atendimentos}} /> : <Simple {...props} {...{expandido, atendimentos}} />}
        </Container>
    )
}
